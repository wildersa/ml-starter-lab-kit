...
- RLRunner (coordinating step/reset execution, transition recording, returns)
...
@dataclass
class StepRecord:
    """Explicit record of a single environment interaction step."""
    episode: int
    step: int
    state_before: Any
...
class BaseAgent:
    ...
    def update(self, transition: StepRecord) -> Dict[str, Any]:
        """
        Updates agent parameters given a completed transition.
...
class RLRunner:
    ...
    Maintains history, cumulative return, episode counters, and manual/auto stepping.
    """
    ...
    def step(self, action: Optional[Any] = None) -> StepRecord:
        ...
        update_info = self.agent.update(record)
        record.update_info = update_info
...
class ToyRLAgent(BaseAgent):
    ...
    def _ensure_obs(self, obs: Any, available_actions: List[Any]) -> Dict[Any, float]:
        obs_key = str(obs)
        if obs_key not in self.q_table:
            self.q_table[obs_key] = {a: 0.0 for a in available_actions}
    ...
    def select_action(self, observation: Any, available_actions: List[Any]) -> Any:
        ...
        best_actions = [a for a, v in q_vals.items() if v == max_v]
    ...
    def get_policy(self, observation: Any, available_actions: Optional[List[Any]] = None) -> Dict[Any, float]:
        ...
        q_vals = self.q_table.get(obs_key, {a: 0.0 for a in available_actions})

        n_actions = len(available_actions)
        if n_actions == 0:
            return {}

        max_v = max(q_vals.get(a, 0.0) for a in available_actions)
        best_actions = [a for a in available_actions if q_vals.get(a, 0.0) == max_v]
    ...
    def update(self, transition: StepRecord) -> Dict[str, Any]:
        ...
        # Ensure entries
        if obs_s not in self.q_table:
            self.q_table[obs_s] = {action: 0.0}
        q_old = self.q_table[obs_s].get(action, 0.0)

        if transition.terminated:
            target = reward
        else:
            next_q_dict = self.q_table.get(obs_next, {})
